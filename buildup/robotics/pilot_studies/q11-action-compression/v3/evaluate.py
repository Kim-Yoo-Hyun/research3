...
def summarize(seeds):
    ...
    gate=json.loads((O/'support.json').read_text())
    decision=decide(codec_valid,len(seeds),gate['decision'],tables,localization)
    common=[r for r in rows if all(r['methods'].get(k,{}).get('status')=='verified' for k in ['fast_joint_gripper','linear_gripper'])]
    delta=[int(r['methods']['linear_gripper']['verification']['success'])-int(r['methods']['fast_joint_gripper']['verification']['success']) for r in common]
    codec_contrast={'episodes':len(common),'linear_minus_joint_success':float(np.mean(delta)) if delta else None,'bootstrap_95':bootstrap(delta),'lost':sum(d<0 for d in delta),'gained':sum(d>0 for d in delta),'interpretation':'approximately matched rate/MSE, not equal budget or a causal contact contrast'}
    result={'decision':decision,'heldout_eligible':len(seeds),'rows':rows,'codec_table':tables,'rate_error_support':comparisons,'localization':localization,
            'no_automatic_hypothesis_selection':True,'pre_intervention_support':gate,'linear_vs_joint':codec_contrast,'post_v2_revision':True}
    (O/'analysis.json').write_text(json.dumps(result,indent=2,allow_nan=False));return result


def decide(measurement_valid,n,gate,tables,localization):
    if not measurement_valid:return 'MEASUREMENT_INVALID'
    if n<P['heldout_min_valid']:return 'STOP_INSUFFICIENT_DENOMINATOR'
    if gate!='READY_FOR_INTERVENTION':return gate
    if not all(t['verified_variants']==n for t in tables.values()):return 'MEASUREMENT_INVALID'
    if tables['fast_joint_gripper']['lost_success']<P['full_codec_min_loss_for_failure_link']:return 'STOP_NO_FAILURE_LINK'
    if not any(r['diagnostic_signal'] for r in localization.values()):return 'STOP_NO_CONTACT_LOCALIZATION_SIGNAL'
    return 'LOCALIZED_DIFFERENCE_REQUIRES_REVIEW'
