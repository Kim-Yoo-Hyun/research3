...
if [ -e "$q11_output" ]; then echo 'Output exists; refusing overwrite' >&2; exit 2; fi
q11_log_dir="$PWD/logs/q11_v2_$(date -u +%Y%m%d_%H%M%S)"
...
timeout 7200 docker run --rm --name research3-q11-pilot --network none --cpus 4 --memory 8g \
